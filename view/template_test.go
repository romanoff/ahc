...
func TestTemplateWidhtDeafultParams(t *testing.T) {
	tmpl := "<div class='button'>{{.name|html}}</div>"
	c := &component.Component{Namespace: "goog.a-button", Template: &component.Template{Content: tmpl}}
	tmpl = `<div class='main-layout'><div class='main-layout-content'>{{.content}}</div></div>`
	c1 := &component.Component{
		Namespace:    "goog.main-layout",
		Template:     &component.Template{Content: tmpl},
...
<div class="container">
  ...
  {{ end }}
	...
	expected := `<!doctype html><div class="container"><div class="main-layout"><div class="main-layout-content"><div class="button">A</div><div class="button">B</div><div class="button">C</div></div></div></div>`
	if expected != string(content) {
		t.Errorf("Expected to get:\n%v\n, but got:\n%v", expected, string(content))
	}
}
